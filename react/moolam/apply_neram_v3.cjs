...
const uiEnd = code.lastIndexOf(`      </Grid>`);

if (uiStart === -1 || uiEnd === -1) throw new Error("Could not find uiStart/End");

const dialogsStart = code.indexOf(`      {/* ----------------------- Export modal`, uiEnd);
const dialogsAndEnd = code.substring(dialogsStart);
...
console.log("Success! Applied Neram styling v3.");

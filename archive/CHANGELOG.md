...
All notable changes to **Elvan Niril** are documented here.
...
- **`Start ElvanNiril.bat`** — default fallback updated to 47371 (was
  ...
  timing).
- **`Stop ElvanNiril.bat`** — default fallback updated to 47371. Still
...
- **`Update ElvanNiril.bat`** — kills the legacy 3001 process *and* the
...
The existing `Update ElvanNiril.bat` script already:
...
- 🟡 **I7 — SIGINT/SIGTERM graceful shutdown.** `Stop ElvanNiril.bat`
...
- **B8 — `Start ElvanNiril.bat` reads `port.txt` before probe.** Real

...
> **TL;DR**
> 1. Double-click **Install ElvanNiril.bat** once. That's it — installer handles the rest.
> 2. A desktop icon appears called *Elvan Niril*. Use it to open the app.
...
1. **Download** the project as a ZIP from
   ...
   `Documents\ElvanNiril`).

2. **Double-click `Install ElvanNiril.bat`.** That's it.
   ...
   - It builds the app.
   - It creates a *Elvan Niril* shortcut on your Desktop and in
     the Start Menu.
...
On the **new** computer:
1. Install the app fresh (run `Install ElvanNiril.bat`).
...
2. **Stop the server first** (close the app, then run *Stop ElvanNiril.bat*
...
A: Run *Update ElvanNiril.bat*. It pulls the latest from GitHub without
...
- **"File came from another computer"** → right-click `Install
  ElvanNiril.bat` → **Properties** → tick **Unblock** at the bottom → OK
...
The local server isn't running. Run *Start ElvanNiril.bat* again, or reboot
...
*Install ElvanNiril.bat* again.

### Update broke something
The previous `data/` folder is untouched, so your bills are safe. Either:
1. Run *Update ElvanNiril.bat* again — most updates self-heal on second run.
...
- Email elvan parthasarathy: contact@elvan parthasarathy.com

...
for file_info in files:
    ...
    start_idx = content.find(file_info["replace_start"])
    end_idx = content.find(file_info["replace_end"], start_idx)

    if start_idx != -1 and end_idx != -1:
        # Extract the part before and after the replacement
        before = content[:start_idx]
        after = content[end_idx + len(file_info["replace_end"]):]
        new_content = before + file_info["replacement"] + after

        # Fix withValues vs withOpacity as Flutter 3.22/Dart 3.4 uses withValues, wait, original used withValues. Let's fix that.
        ...
        print(f"Failed to find block in {file_info['name']}")

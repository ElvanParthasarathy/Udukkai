...
const int maxKooliProfiles = 5;

final kooliNiruvanaTharavugalListProvider =
    NotifierProvider<KooliNiruvanaTharavugalListNotifier, List<NiruvanaTharavugal>>(() {
  return KooliNiruvanaTharavugalListNotifier();
});

final kooliNiruvanaTharavugalProvider = Provider<NiruvanaTharavugal?>((ref) {
  final profiles = ref.watch(kooliNiruvanaTharavugalListProvider);
  final prefs = ref.watch(sharedPreferencesProvider);

  if (profiles.isEmpty) return null;

  final activeId = prefs.getInt('coolie_active_profile_id');
...
class KooliNiruvanaTharavugalListNotifier extends Notifier<List<NiruvanaTharavugal>> {
  late KooliDatabase _db;
  late NirilBackupService _backupService;
  late dynamic _prefs; 

  @override
  List<NiruvanaTharavugal> build() {
    _db = ref.watch(kooliDatabaseProvider);
  ...
  Future<void> hideProfile(int id) async {
    await (_db.update(_db.kooliNiruvanaTharavugalTable)..where((t) => t.id.equals(id)))
        .write(KooliNiruvanaTharavugalTableCompanion(isDeleted: const Value(true)));
    await _loadAllProfiles();
  }

  Future<bool> restoreProfile(String kurumPeyar) async {
    final hiddenProfile = await (_db.select(_db.kooliNiruvanaTharavugalTable)
    ...
    if (hiddenProfile != null) {
      await (_db.update(_db.kooliNiruvanaTharavugalTable)..where((t) => t.id.equals(hiddenProfile.id)))
          .write(KooliNiruvanaTharavugalTableCompanion(isDeleted: const Value(false)));
      await _loadAllProfiles();
      return true;
    }
    return false;
  ...
  Future<void> _loadAllProfiles() async {
    ...
    try {
      final entries = await (_db.select(_db.kooliNiruvanaTharavugalTable)
            ..where((t) => t.isDeleted.equals(false)))
          .get();
      state = entries.map(_entryToModel).toList();
    } catch (e) {
      print('Error loading Kooli profiles: $e');
      state = [];
    }
  }

  Future<int?> createProfile(NiruvanaTharavugal profile) async {
    if (state.length >= maxKooliProfiles) return null;

    final id = await _db.into(_db.kooliNiruvanaTharavugalTable).insert(
          _modelToCompanion(profile),
        );

    profile.id = id;
    state = [...state, profile];
    _prefs.setInt('coolie_active_profile_id', id);
  ...
  Future<void> updateProfile(NiruvanaTharavugal profile) async {
    if (profile.id != null) {
      await (_db.update(_db.kooliNiruvanaTharavugalTable)
  ...
  Future<void> deleteProfile(int id) async {
    await (_db.delete(_db.kooliNiruvanaTharavugalTable)
          ..where((t) => t.id.equals(id)))
        .go();
    state = state.where((p) => p.id != id).toList();

    final activeId = _prefs.getInt('coolie_active_profile_id');
    if (activeId == id) {
      if (state.isNotEmpty) {
        _prefs.setInt('coolie_active_profile_id', state.first.id!);
      } else {
        _prefs.remove('coolie_active_profile_id');
      }
    }
    _backupService.createBackup();
  }
  ...
  Future<void> clearProfile() async {
    await _db.delete(_db.kooliNiruvanaTharavugalTable).go();
    state = [];
    _prefs.remove('coolie_active_profile_id');
  }

  NiruvanaTharavugal _entryToModel(KooliNiruvanaTharavugalEntry entry) {
    return NiruvanaTharavugal(
      id: entry.id,
      mudhanMozhi: 'ta',
      thunaiMozhi: 'en',
      iruMozhi: true, // Coolie is hardcoded to bilingual
      ...
      upiId: entry.upiId,
      thoatraNiram: entry.thoatraNiram,
    );
  }

  KooliNiruvanaTharavugalTableCompanion _modelToCompanion(NiruvanaTharavugal model) {
    return KooliNiruvanaTharavugalTableCompanion(
      ...
      thoatraNiram: Value(model.thoatraNiram),
    );
  }
}

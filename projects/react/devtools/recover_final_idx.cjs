...
const shareEmailIdx = content.indexOf('const shareEmail = (bill) => {');
const h3Idx = content.indexOf('<h3 className="client-card-name">{clientName}</h3>');

if (shareEmailIdx !== -1 && h3Idx !== -1) {
  const replacement = `const shareEmail = (bill) => {
  ...
  content = content.substring(0, shareEmailIdx) + replacement + content.substring(h3Idx);
